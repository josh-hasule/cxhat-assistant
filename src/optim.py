...
Further contributins from @karpathy and @chrisjmccormick. 
"""
...
def adamw_step_fused(
    ...
    p.mul_(1 - lr_t * wd_t)
    # upddate running average (lerp_ is cleaner and fuses well)
...
Background:
Newton-Schulz iteration to compute the zeroth power / orthonalization of G. We opt to use a 
quintic iteration whose coefficients are selected to maximize the slope at zero. For the purpose 
of minimizing steps, it turns out to be empirically effective to keep increasing the slope at 
zero even beyond the point where the iteration no longer converges all the way to one everywhere 
on the interval. This iteration therefore does not produce UV^T but rather something like US'V^T
where S' is diagonl with S_{ii}' ~ uniform(0.5, 1.5), which turns out not to hurt model 
...
# Coefficients for Polar Express (computed fro num_iters=5, safety_factor=2e-2, cushion=2)
...
def muon_step_fused(
    ...
    # Cautios weight decay + parameter update 
    lr = lr_t.to(g.dtype)
    wd = wd_t.to(g.dtype)
